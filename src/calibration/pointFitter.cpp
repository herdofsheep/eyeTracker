...
void pointFitter::setup(int _nDivisionsWidth, int _nDivisionsHeight) {
	...
	referencePoints.assign(nDivisionsWidth * nDivisionsHeight , emptyPoint);
	pointsFromls.assign(nDivisionsWidth * nDivisionsHeight , emptyPoint);
...
void pointFitter::calculateErrors(ofRectangle & calibrationRectangle){
	...
	for (int i = 0; i < nDivisionsHeight; i++) {
		for (int j = 0; j < nDivisionsWidth; j++) {
			...
			referencePoints[j+i*nDivisionsWidth].x = calibrationRectangle.x + ((float)calibrationRectangle.getWidth() / (float)(nDivisionsWidth-1)) * jj;
			referencePoints[j+i*nDivisionsWidth].y = calibrationRectangle.y + calibrationRectangle.getHeight()
													- ((float)calibrationRectangle.getHeight() / (float)(nDivisionsHeight-1)) * i;
...
void pointFitter::drawNonCalibration(ofRectangle & calibrationRectangle) {
	...
	ofSetColor(255,0,0);
	ofNoFill();
	ofRect(calibrationRectangle.x, calibrationRectangle.y, calibrationRectangle.getWidth(), calibrationRectangle.getHeight());
	ofPopStyle();
}
