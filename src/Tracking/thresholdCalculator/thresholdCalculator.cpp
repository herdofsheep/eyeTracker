...
void thresholdCalculator::setup(int width, int height, float _divisor){
	...
	farFromAvg = cvCreateImage(cvSize(roi.getWidth(), roi.getHeight()), IPL_DEPTH_8U, 1);
	newMask = cvCreateImage(cvSize(roi.getWidth(), roi.getHeight()), IPL_DEPTH_8U, 1);
...
void thresholdCalculator::drawPupilImageWithScanLine(int x, int y, int w, int h, ofxCvGrayscaleImage & img) {
	...
	ofFill();
	ofCircle(whiteLocMin.x * (img.width / roi.getWidth()), whiteLocMin.y * (img.height / roi.getHeight()), 10);
...
{
	ofPixels_<unsigned char> tempPixels = img.getPixels();
