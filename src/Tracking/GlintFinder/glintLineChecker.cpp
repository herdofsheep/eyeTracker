...
void glintLineChecker::update(ofxCvGrayscaleImage & eyeImg, int nGlints, ofxCvContourFinder & contourFinder, bool bUseGlintInBrightEye, ofxCvContourFinder & contourFinderBright){
	...
	if (nGlints == 2) {
		...
		lineSegments.clear();
		ofPixels pixels = eyeImg.getPixels();
...
//			lineSegments.erase(remove_if(lineSegments.begin(), lineSegments.end(), glintChecker::lineInRange), lineSegments.end());
		...
		ofPixels stripepixels = myStripesImage.getPixels();
		...
		if (nBlobs > 0) {
			...
			for (int i = 0; i < contourFinder.blobs.size(); i++){
				...
				if (ofInRange(foundLinesRect.x, blobRect.x, blobRect.x + blobRect.getWidth()) &&
					(ofInRange(foundLinesRect.y, blobRect.y, blobRect.y + blobRect.getHeight()) ||
					 ofInRange(foundLinesRect.y + foundLinesRect.getHeight(), blobRect.y, blobRect.y + blobRect.getHeight()))){
						leftGlintID = i;
					} else if (ofInRange(foundLinesRect.x + foundLinesRect.getWidth(), blobRect.x, blobRect.x + blobRect.getWidth()) &&
							   (ofInRange(foundLinesRect.y, blobRect.y, blobRect.y + blobRect.getHeight()) ||
								ofInRange(foundLinesRect.y + foundLinesRect.getHeight(), blobRect.y, blobRect.y + blobRect.getHeight()))) {
						rightGlintID = i;
					}
