	...
	for (int k = 0; k < 2; k++) {
		for (int i = 0; i < 2; i++) {
			...
			for (int j = 0; j < glintHis[k].glints[i].size(); j++) {
				...
				tempAvgWidth += glintHis[k].glints[i][j].boundingRect.getWidth();
				tempAvgHeight += glintHis[k].glints[i][j].boundingRect.getHeight();
			}
		...
		for (int j = 0; j < glintHis[k].glints[GLINT_ELEMENT_LEFT].size(); j++) {
			tempAvgDistance += glintHis[k].glints[GLINT_ELEMENT_RIGHT][j].boundingRect.x
							...
							+ glintHis[k].glints[GLINT_ELEMENT_LEFT][j].boundingRect.getWidth());
		}
...
bool glintRefChecker::checkSize(ofxCvBlob & blob, int glintID, float minPct, float maxPct) {
	...
	if (ofInRange(blob.boundingRect.getWidth(), glintHis[vertPos].avgWidth[horiPos] * minPct, glintHis[vertPos].avgWidth[horiPos] * maxPct)
		&& ofInRange(blob.boundingRect.getHeight(), glintHis[vertPos].avgHeight[horiPos] * minPct, glintHis[vertPos].avgWidth[horiPos] * maxPct)){
		return true;
	} else {
		return false;
	}
